...
const { CREATE_TASK_USER } = require("../locales/messages")
...
exports.create = async (req, res) => {
    try {
        ...
        if (olduser) {
            return sendError(res, errormessages.EMAIL_ALLREADY_EXSIST, statusCode.ERRORCODE)
        ...
        sendScuccess(res,
            { message: messages.CREATE_NEW_USER, user: { _id: user._id, name: user.name, email: user.email } },
            statusCode.CREATE)
    } catch (e) {
        console.log(e);
        res.send(e)
...
exports.verifyEmail = async (req, res) => {
    try {
        ...
        if (user.isVerified) {
            return sendError(res, errormessages.USER_ALLREADY_VERIFIED, statusCode.ERRORCODE)
        }
        ...
        if (!token) {
            return sendError(res, errormessages.TOKEN_NOT_FOUND, statusCode.ERRORCODE)
        }

        const isMatched = await token.comparetoken(otp)

        if (!isMatched) {
            return sendError(res, errormessages.OTP_NOT_MATCHED, statusCode.ERRORCODE)
        }
...
exports.resendEmailVerificationToken = async (req, res) => {
    ...
    if (allreadyhastoken) {
        return sendError(res, errormessages.ALL_READY_SEND_OTP, statusCode.ERRORCODE)
    }
    ...
    sendScuccess(res, { message: messages.OTP_SENT_TO_EMAIL }, statusCode.SUCCESS)
...
exports.forgetPassword = async (req, res) => {
    try {
        ...
        if (!email) {
            return sendError(res, errormessages.EMAIL_IS_MISSING, statusCode.ERRORCODE)
        }
...
exports.resetpassword = async (req, res) => {
    ...
    if (matched) {
        return sendError(res, errormessages.PASSWORD_MATCH_WITH_OLDPASSWORD, statusCode.ERRORCODE)
    }
...
exports.signIn = async (req, res) => {
    try {
        ...
        if (!user) {
            return sendError(res, errormessages.INVALID_LOGIN_CREDENTIALS, statusCode.ERRORCODE)
        }
